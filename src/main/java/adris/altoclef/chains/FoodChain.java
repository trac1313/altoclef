...
public class FoodChain extends SingleTaskChain {
    ...
    public boolean needsToEat() {
        ...
        if (foodLevel < _config.alwaysEatWhenBelowHungerAndPerfectFit && _cachedPerfectFood.isPresent()) {
            ...
            ItemStack best = _cachedPerfectFood.get().getDefaultStack();
            int fills = (best.get(DataComponentTypes.FOOD) != null) ? best.get(DataComponentTypes.FOOD).nutrition() : -1;
            return fills == need;
        }
    ...
    private Pair<Integer, Optional<Item>> calculateFood(AltoClef mod) {
        ...
        for (ItemStack stack : mod.getItemStorage().getItemStacksPlayerInventory(true)) {
            if (stack.contains(DataComponentTypes.FOOD)) {
                ...
                foodTotal += Objects.requireNonNull(stack.get(DataComponentTypes.FOOD).nutrition() * stack.getCount());
            }
        }
